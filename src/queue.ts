...
import { isArray, isInteger, isObject } from '@benzed/types'
...
function isQueuePayload<V, T extends object | void>(
    ...
    return (
        isObject<{ [key: string]: unknown }>(input) &&
...
interface QueueOptions {
    /**
     * Number of items the queue will run simultaneously.
     *
     * Defaults to 1.
     */
...
type QueueItem<V, T extends object | void> = (T extends void
    ? {
          /**/
      }
    : T) & {
    /**/ readonly [K in keyof QueueState<V, T> as K extends 'task' | 'complete'
        ? /*    */ never
        : /*    */ K]: QueueState<V, T>[K]
} & {
...
type QueueAddInput<V, T extends object | void> = T extends void
    ? QueueTask<V, T>
    : { task: QueueTask<V, T> } & T

//// Queue ////

class Queue<V = void, T extends object | void = void> extends EventEmitter<
    QueueEvents<V, T>
> {
    private readonly _queued: LinkedList<{
        item: QueueItem<V, T>
    ...
    private readonly _current: LinkedList<{
        item: QueueItem<V, T>
        state: QueueState<V, T>
    }> = new LinkedList()
     ...
     */
    add(task: QueueAddInput<V, T>): QueueItem<V, T>
    ...
    add(input: QueueAddInput<V, T> | QueueAddInput<V, T>[]): unknown {
        ...
        if (this.numTotalItems + tasks.length > this.maxTotalItems)
            throw new Error(
                `Cannot queue more than ${this.maxTotalItems} items.`
            )
    ...
    private _createQueuedItem(input: QueueAddInput<V, T>): QueueItem<V, T> {
        const { task, ...data } =
            typeof input === 'function' ? { task: input } : input
        ...
        const item = {
            ...
            complete: () => {
                const promise = (state.complete ??= new Promise(
                    (resolve, reject) => {
                        const onComplete = (): void => {
                            if (state.stage !== 'complete') return

                            this._removeListener('complete', onComplete, {
                                internal: true
                            })
                            this._removeListener('error', onComplete, {
                                internal: true
                            })

                            if (state.error) reject(state.error)
                            else resolve(state.result?.value as V)
                        }

                        this._addListener('complete', onComplete, {
                            internal: true
                        })
                        this._addListener('error', onComplete, {
                            internal: true
                        })
     ...
     */
    remove(item: QueueTask<V, T> | QueueItem<V, T>): number {
        ...
        while (indexes.length > 0) this._queued.remove(indexes.pop())
    ...
    private async _updateCurrentItems(): Promise<void> {
        while (
            ...
            this._current.append(this._queued.remove())
        }
    ...
    private _removeCurrentItem(current: {
        item: QueueItem<V, T>
        state: QueueState<V, T>
    }): void {
        const index = this._current.indexOf(current)
        if (index < 0) throw new Error('Item is not currently executing.')

        this._current.remove(index)
    }

    private async _executeCurrentItem(current: {
        item: QueueItem<V, T>
        state: QueueState<V, T>
    }): Promise<void> {
        const time = new Date()
        ...
        try {
            ...
            if (this._current.has(current)) this._removeCurrentItem(current)
