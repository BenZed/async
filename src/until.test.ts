...
describe('until', () => {
            ...
            it('maximum time, in milliseconds, while loop can run', async () => {
                ...
                try {
                    ...
                    expect((e as Error).message).toContain(
                        'Could not resolve condition in 20 ms'
                    )
                }
            })
            ...
            it('message to throw in error if while loop times out', async () => {
                ...
                try {
                    await until(() => false, {
                        ...
                        timeout: 10,
                        timeoutMsg:
                            'if it cannot be done in 10 milliseconds, it cannot be done'
                    })
                } catch (e: unknown) {
                    expect((e as Error).message).toContain(
                        'if it cannot be done in 10 milliseconds, it cannot be done'
    ...
    it('returns number of milliseconds waiting took', async () => {
        const ms = await until((delta: number) => delta > 10, {
            interval: 1
        })
