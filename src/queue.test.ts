...
describe('maxTotalItems option', () => {
    it('Must be 1 or higher', () => {
        for (const badNumber of [0, -1, NaN]) {
            expect(() => new Queue({ maxTotalItems: badNumber })).toThrow(
                'options.maxTotalItems must be 1 or higher.'
            )
        }
    })

    it('Must be a whole number', () => {
        expect(() => new Queue({ maxTotalItems: 1.5 })).toThrow(
    ...
    it('Error is thrown if too many items are queued', () => {
        ...
        expect(() => queue.add(jest.fn<Func>())).toThrow(
            'Cannot queue more than 2 items.'
        )
    })
})
...
describe('maxConcurrent option', () => {
    it('Must be 1 or higher', () => {
        for (const badNumber of [-1, 0]) {
            expect(() => new Queue({ maxConcurrent: badNumber })).toThrow(
                'options.maxConcurrent must be 1 or higher.'
            )
        }
    })
    ...
    it('Must be a whole number', () => {
        for (const badNumber of [1.5, Infinity]) {
            expect(() => new Queue({ maxConcurrent: badNumber })).toThrow(
                'options.maxConcurrent must be an integer.'
            )
        }
    })
    ...
    describe('execute tasks concurrently', () => {
        ...
        beforeAll(async () => {
            ...
            for (let taskId = 0; taskId < TOTAL_TASKS; taskId++) {
                ...
                queue.on('start', payload => {
                    if (item === payload.item) addTaskState(taskId, '🏃')
                })
        ...
        it('Runs every item in the queue', () => {
            const stateIndexesThatContainRun = Array.from(
                { length: TOTAL_TASKS },
                (_, i) => taskStates.some(state => state[i] === '🏃')
            )

            expect(
                stateIndexesThatContainRun.every(state => state === true)
            ).toBe(true)
        })
    })
})
...
describe('add()', () => {
    ...
    it('returns items added', () => {
        expect(items.every(item => queue.queuedItems.includes(item))).toBe(true)
    })
...
describe('remove()', () => {
    ...
    it('removes items from the queue via task task input', () => {
        expect(
            queue.queuedItems.some(
                item => item === items[1] || item === items[3]
            )
        ).toBe(false)
    })
...
describe('events', () => {
    ...
    it('"complete" event second argument is value', async () => {
        const callback = jest.fn<Func>()
    ...
    it('.complete() promises resolve when already finished', async () => {
        ...
        await Promise.all([item.complete(), queue.complete()])
